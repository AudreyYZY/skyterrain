...
| 地形讲解 | 1377 × 6 段 × 2 语言 | **1302 个条目至少一字段带核实留痕（2026-09-21 机械实测）** | `npm run gen:verify-status` 口径；按字段只有 **1413 / 8262（17%）** 带核实留痕。⚠️ 这是**覆盖率不是正确率**，也不能把“条目有一字段核过”说成整条六段都核过；正确率仍只看均匀随机抽样 |
