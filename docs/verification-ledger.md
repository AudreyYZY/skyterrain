...
## 覆盖现状（截至 2026-09-20；各行具体时点见说明）

| 内容 | 总数 | 已核实 | 说明 |
|---|---|---|---|
| 航线航班信息 | 280 | **280（全部）** | 逐条上网核实完毕：91 条属实、163 条已改正、26 条有误无替代（不显示航班号）。另有 15 条无不经停航班的航线整条移除（295 → 280）|
| 航线解说 | 280 × 2 套（学习 / 旅游）× 2 语言 | **280 条（全部，2026-09-11 实测）** | 同一计法：条目块内出现 `sources:` 留痕即算核过。三波 13 批（R23a–R26e）：361 条结论里 265 ok、94 处改正（落在 70 条航线）、2 处开 issue。距离、承运人、机型、汉字数字四类此后由 `check:distance` / `check:routes` / `check:zhen` 离线守住 |
| 地形讲解 | 1370 × 6 段 × 2 语言 | **1295 个条目至少一字段带核实留痕（2026-09-20 机械实测）** | `npm run gen:verify-status` 口径；按字段只有 **1406 / 8220（17%）** 带核实留痕。⚠️ 这是**覆盖率不是正确率**，也不能把“条目有一字段核过”说成整条六段都核过；正确率仍只看均匀随机抽样 |
| 机场名与现状 | 513 个 IATA（注册表）+ 9 个航线第二机场 | **522 个（全部，2026-09-11）** | 名称、是否已迁建 / 关闭 / 停客运、IATA 对应；坐标：四批补核逐个比对，南欧 39 个 ok 项未逐个比对。改了 10 座机场、开 4 个 issue（R27a–d）。境外机场只写描述性名称、不追冠名；中国机场按民航局正式名称 |
