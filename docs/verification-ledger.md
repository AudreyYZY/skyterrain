...
lastVerifiedCommit: 155278f

## 覆盖现状（截至 2026-09-23；各行具体时点见说明）

| 内容 | 总数 | 已核实 | 说明 |
|---|---|---|---|
| 航线航班信息 | 303 | **303 条均有来源状态** | 289 条可展示核实后的航班信息；5 条 `wrong` + 9 条 `flight-unknown` 不显示航班号或机型。2026-09-07 首轮 280 条审计为 91 属实 / 163 改正 / 26 无替代；后来新增航线逐条留 `source`（含 2026-09-22/23 南非 6 条） |
| 航线解说 | 303 × 2 套（学习 / 旅游）× 2 语言 | **303 条均有 `sources` 留痕** | 距离、承运人、机型、汉字数字由 `check:distance` / `check:routes` / `check:zhen` 离线守住；`flight-unknown` 解说不得点名航班号、机型或时长 |
| 地形讲解 | 1402 × 6 段 × 2 语言 | **1302 个条目至少一字段带核实留痕（2026-09-22 机械实测）** | `npm run gen:verify-status` 口径。⚠️ 这是**覆盖率不是正确率**，也不能把“条目有一字段核过”说成整条六段都核过；正确率仍只看均匀随机抽样。⚠️ **南非批 1/批 2/批 3 的「⚠️ 核实更正（日期）」措辞不匹配生成脚本的 `VERIFY_MARK` 正则**（该正则只认「核实 YYYY-MM」/「YYYY-MM-DD 核」/「级别 N」），导致这些条目在本表里被计成未核实——已提出为待修的工具 bug，见任务队列，不影响内容本身的核实状态（核实结论已落在 `docs/verification-ledger.md` 历史表与 `docs/africa-country-selection-2026-09-18.md`） |
| 机场名与现状 | 582 个城市注册表 IATA + 10 个仅作航线端点的机场 | **592 个由 `check:places` / `check:routes` 持续比对** | 名称、IATA、坐标和条目来源由门禁检查；境外机场只写描述性名称、不追冠名；中国机场按民航局正式名称 |
| 城市讲解 | 1014 篇（961 城 + 53 国家概览）× 最多 7 段 × 2 语言 | **837 篇至少一字段带核实留痕（2026-09-22 机械实测，未计入 2026-09-23 新增 2 城）** | 按字段只有 **1371 / 7045（19%）** 带核实留痕；`identity` / `layout` 仍是数字失准重灾区 |
...
### 2026-09-19 · 纳米比亚地形批 1（R46）的三条方法学结论

**① 核实方漏报的那一处，是「拿错的值全库 grep」捞出来的。** 核实方判出埃龙戈的环状岩脉写成了
「辉长岩」（GSN 原文是 dolerite 辉绿岩），但只报了 `erongo-mountains/formation` 一处；
按既有规矩拿**错的值**去 grep，发现**布兰德贝格的 `distinguish` 段里还有第二处**同样的错。
→ 这条规矩（不是拿正确值搜、是拿错的值搜）在本轮再次证明有效，**核实方给的命中位置不能当作全集**。

**② 两处「1 级来源互相冲突」，处理方式不同，判据是「对称不对称」：**
- **不对称 → 不并列**：索苏斯维来的沙丘是不是「世界最高」——MEFT 旅游页说是、同一国政府署名的 UNESCO
  申报文本说**不是**并解释了为什么（营销说法、跨区域沙丘高度比较不出现在地貌学文献里）。
  后者解释了前者的成因，**直接不写**，不并列。
- **对称 → 改写成覆盖两者的区间**：鱼河峡谷的德怀卡冰期年代，GSN 写 ~350 Ma、UNESCO TL 1745 写 ~300 Ma，
  两者互不解释。正文改成「约 3 亿到 3.5 亿年前」，**不选边**，并在注释里写明两个出处。
  （布兰德贝格的相对高差 >2000 m / >1800 m 同理，取两者都成立的保守下界。）

**③ 「无 1 级出处的派生数字」要降级，不要因为它看起来精确就留着。**
布兰德贝格原写「东西 30 公里 × 南北 27 公里」，核实发现 GSN 与 TL 1744 **都没给任何尺寸数**——
那两个数是量 OSM 多边形外接矩形得到的 3 级派生值。改写成「近圆形、直径约三十公里」，
与 TL 的 almost circular 和 2 级面积（约 650 km²，等效直径约 28.8 km）相容。
**一个比来源更精确的数字，精确的那部分是自己造的。**

## 历史记录

| 日期 | 范围 | 条数 | ok / wrong / unknown | 结果 |
|---|---|---|---|---|
| 2026-09-23 | **南非旅游模式批 3**（新增 2 城：姆邦贝拉/鲁斯滕堡，补齐九省全覆盖；Stats SA Census 2022 Municipal Fact Sheet 原始 PDF + Wikipedia + 多源公路里程站交叉核对）（R57） | 约 25 | 23 / 2 / 0 | 修 2 处真错：姆邦贝拉人口最初写 818,925（citypopulation.de 转引的旧数），核实方直接读取 Stats SA 原始 PDF 发现开篇勘误声明——姆邦贝拉与塔巴楚韦两市首次发布数已被修正，正确数字 809,674，已改正；鲁斯滕堡"最近有定期航班的机场是约112公里外的OR·坦博国际机场"两头都错，实际公路里程约173公里，且真正最近的是兰赛里亚机场（约107公里），已改正。另有一处低置信度修正：姆邦贝拉到神之窗车程原估75-90公里偏低，改为90-106公里（核实方认为原有的"不作本市地标、仅一日往返提及"框架本身恰当，只是数字偏保守）。核实确认：姆邦贝拉2016年乌姆津迪并入、鲁斯滕堡2021年无党过半治理、两市各自与区自治市共驻一城的结构性事实均属实；零红线命中——鲁斯滕堡矿业仅作中性经济表述，2012年马里卡纳事件全程未涉及；新开 issue 0、关闭 0 |
| 2026-09-23 | **南非航线批 2**（3 条国内航线：约翰内斯堡—金伯利/乌平通、开普敦—乔治；Airportia/AirNav Radar/flightmapper.net/PlaneFinder/PlaneMapper 逐项核对，特别复核上一批的航班号方向坑与地貌坑）（R56） | 约 20 | 20 / 0 / 0 | 首次「零错误」的南非航线批次。独立重新核实三个航班号方向（4Z723 JNB→KIM、4Z763 JNB→UTN、4Z621 CPT→GRJ 均以实际起降时刻确认，不靠单双号推断），全部与初稿一致；机型 jnb-kim「Embraer 140」与 jnb-utn「Embraer 135」经尾号追溯确认未互换；cpt-grj 机型冲突（核实中又发现第三个来源给出 E190，与初稿已知的 ERJ-135/E195-E2 冲突三方不一致）确认维持泛化表述是正确处理、不应强行锁定；弗里德堡陨石坑地貌描述与已发布 `vredefort-dome` 地形条目一致，几何上复核确认航线确实经过（约3.6公里内）；乔治虽有可靠来源支持「地处乌坦尼夸山麓」，核实方确认本批刻意维持泛化表述是恰当保守而非缺陷；零红线命中；新开 issue 0、关闭 0 |
| 2026-09-22 | **南非航线批 1**（3 条国内航线：约翰内斯堡—布隆方丹/波罗奎瓦内/理查兹湾；Flightera/PlaneFinder/Airportia/FlightAware/flyteam.jp/理查兹湾机场官网时刻表逐项核对，含承运人重名误判排查）（R55） | 约 18 | 12 / 6 / 0 | 修 6 处真错：三条航线的航班号全部方向填反——Airlink 按单双号区分往返（奇数=JNB出发、偶数=返程），jnb-bfn 的 4Z706、jnb-ptg 的 4Z802、jnb-rcb 的 4Z884 均误用了返程号，已分别改正为去程号 4Z705/4Z801/4Z883（机型不受影响）；jnb-ptg study 段落「波罗奎瓦内坐落于苏茨山脉南麓山麓谷地」有误，该市实际在海拔约1230米的台地上，真正的山麓城市是逾百公里外的路易特里查德，已改写为台地城市描述（中英文各一处）。距离/时长量级、机场三字码、布什维尔德杂岩体地质表述、高地草原/沿海低地过渡描述、零争议红线扫描、理查兹湾"非洲重要煤炭出口港之一"、布隆方丹"三首都之一（司法首都）"均核实属实无需改动。核实过程中发现 `pvg-dub`（爱尔兰）旅游解说提到的都柏林机场接驳巴士品牌「Airlink」与南非这家支线航司同名，被承运人比对脚本误判为点名错误承运人，判明是重名巧合后加入 `scripts/check-routes.ts` 的 `BENIGN` 豁免表，不属内容错误；新开 issue 0、关闭 0 |
| 2026-09-22 | **南非旅游模式批 2**（新增 7 城：埃库尔勒尼/东伦敦/金伯利/乔治/理查兹湾/乌平通/克尼斯纳；Stats SA Census 2022 官方 PDF + Wikipedia + SAHO 逐项核对，含东伦敦改名/金伯利矿业史红线专项检查）（R54） | 约 30 | 27 / 3 / 0 | 修 3 处真错：埃库尔勒尼「都会自治市人口第三」应为「第四」（约翰内斯堡/开普敦/德班之后）；东伦敦建城史多处有误——「次年（1837）建格拉摩根堡」应为「1847-48年」，「并入并定名东伦敦港」应为「最初命名雷克斯港、1848年1月由开普总督哈里·史密斯正式并入并改名东伦敦」，「因水牛群得名」的城市名词源有误（那是水牛河的词源，不是"East London"本身的词源，已改为「东方的伦敦」这一更可靠说法）；理查兹湾「印度出口占比35%-40%」偏低，2024年实际约占总量近一半，已订正。东伦敦2026年「KuGompo City」改名争议核实确认措辞恰当（不高估为已定、不低估为不存在，仅陈述行政事实不选边）；金伯利矿业史核实确认零命中「封闭工棚制」；红线扫描零命中种族隔离/族群/政治议题；新开 issue 0、关闭 0 |
| 2026-09-22 | **南非旅游模式批 1**（1 国家概览 + 8 城，全库首批南非旅游内容；Stats SA Census 2022 官方 PDF + Wikipedia 逐项核对，含索维托/格贝哈红线专项检查）（R53） | 约 30 | 27 / 3 / 0 | 修 3 处真错：地方自治市数量「226」是 2016 年区划改革前旧数，改 205；国家概览一度把约翰内斯堡写成「豪登省最大城市却不是省会」，与约翰内斯堡条目自身「是豪登省省会」矛盾——约翰内斯堡确实是省会（省议会驻约翰内斯堡市政厅），比勒陀利亚是国家行政首都但不是豪登省省会，已订正、德班/彼得马里茨堡例证保留；「OR·坦博是非洲最繁忙机场」已过期（2025年开罗反超），改「本国客流量最大」。另有斯泰伦博斯「近五分之一葡萄种植面积」二级来源冲突，弱化为不给精确分数的表述。索维托核实结论——不单列词条，理由与批3排除里希特斯费尔德/人类摇篮同一逻辑；约翰内斯堡条目仅一句纯行政沿革式提及，核实确认零命中1976年起义等政治叙事；格贝哈机场命名人物大卫·斯图尔曼与「纳尔逊曼德拉湾」市镇名均只作行政名称客观引用，核实确认零展开人物生平；新开 issue 0、关闭 0 |
| 2026-09-22 | **南非地形批 4**（1 条：梅林斯普特峡谷，凑满单国完成线 25 条；GeoExpro + Wikipedia + Swartberg Circle Route 逐项核对）（R52） | 11 | 9 / 0 / 1 | 全数属实，零 wrong。1 项 unknown 是中文译名「梅林斯普特峡谷」「斯瓦特山脉」——核实方检索未找到权威中文一级来源，条目自身已诚实标注这一局限，无需改动。「约 3.3 亿年前碰撞造山启动」与已收录塞德堡条目「约 3 亿至 2.5 亿年前」峰期褶皱窗口经核实确认不矛盾（俯冲启动早于峰期造山数千万年，正文措辞已把区别说清楚）；南非地形自此达到 25 条完成线；新开 issue 0、关闭 0 |
| 2026-09-22 | **南非地形批 3**（7 条：德胡普维雷湖/爱德华王子群岛/索特潘斯山脉/沃特堡/克鲁格低地/布劳山/厄加勒斯角，Ramsar 官方数据表 + Wikipedia + 学术锆石定年逐项核对）（R51） | 7 | 7 / 0 / 0 | 全数属实，零 wrong——本项目少见的「零错误」批次。当场按核实方建议的两处轻量校正：克鲁格低地西部花岗岩年代「约 35 亿年」偏乐观，改用内尔斯普雷特花岗岩基岩锆石定年「约 32 亿年」（1/2 级学术数据）；布劳山自然保护区设立年份两说冲突（1983 年多个旅游来源 vs Wikipedia 保护区条目信息框给出 1990 年）未消解，正文改为并列两说。德胡普维雷湖对 Ramsar 官方 1984 年数据表「4000 million years」笔误的更正（改为约 4 亿年）经核实确认合理；爱德华王子群岛「无国际主权争议」的红线专项核实同样确认无误；新开 issue 0、关闭 0 |
| 2026-09-21 | **南非地形批 2**（7 条：纳马夸兰/伊西曼格利索/高地草原/齐齐卡马/狂野海岸/马加利斯堡/布什维尔德杂岩体，UNESCO 提名文件 914.pdf + 学术文献逐项核对，含 Wild Coast 红线专项检查）（R50） | 7 | 7 / 0 / 0 | 全数属实，零 wrong。Wild Coast 红线检查通过：对正文全文做关键词扫描（科萨/曼德拉/姆贝基/特兰斯凯等），零命中，仅 registry 警示注释本身命中。中英数字交叉核对无不一致；新开 issue 0、关闭 0 |
| 2026-09-21 | **南非地形批 1**（非洲第二国，10 条，UNESCO 提名文件 1162.pdf/985.pdf + 学术论文逐项核对）（R49） | 37 | 32 / 4 / 1 | 当场修 4 条：德拉肯斯岩画统计数字拼接自两套不同来源、改回 985.pdf 原文「约 600 处遗址、逾 35,000 幅图像」；巴伯顿「第二古老裸露岩石」窄化为「第二古老生命证据保存地」（加拿大阿卡斯塔片麻岩更古老）；卡鲁博福特群误配三叠纪末灭绝事件（该记录实际在斯托姆贝格群）；金门高地初建面积 47.92→17.92 km²（数字误录）+ 扩大年份 1981→1980。1 条 unknown（巴伯顿 1575.pdf 提名文件编号仍 404，暂用 IUCN World Heritage Outlook 2 级来源替代）。另按核实方指出的红线一致性问题，把奥兰治河的邻国具名改写为中性表述（与既有的克罗地亚/斯洛伐克/斯洛文尼亚判例对齐）；新开 issue 0、关闭 0 |
| 2026-09-21 | **纳米比亚完成批**：7 地形 + 6 城 + 2 航线，以 NSA / GSN-MIME / UNESCO / MEFT / 航司官方材料复核关键数字、分类、机场与航点关系（R48） | 15 | 15 / 0 / 0 | 门禁抓出 6 个中文人口句缺少“普查口径”和 1 处距离缺口径，均当场补清；机场坐标、正文距离、中英数字、航线距离无异常；#313 / #314 根因修复 |
| 2026-09-20 | **纳米比亚地形批 2**（8 条新增内容，中英逐项对照一级来源与条目内自洽性）（R47） | 2 | 0 / 2 / 0 | 当场修 2 条：删卡拉哈里“没有国家公园覆盖”的错误概括；修库伊塞布“单一下切”与多期填充—再切自相矛盾；新开 issue 0、关闭 0 |
| 2026-09-19 | **纳米比亚地形批 1**（非洲首批，10 条，两批 content-verifier 各 5 条，中英各核）（R46） | 4 | 0 / 3 / 1 | 当场修 3 条（含 1 条核实方没报、靠「拿错值全库 grep」捞出的第二处）；1 条 unknown 降级改写；新开 issue 3（#313 #314 #315）、关闭 0 |
| 2026-09-12 | 匈牙利 8 城当批核实（18 项，14 属实）（R45） | 5 | 0 / 5 / 0 | 当场修 5 条；新开 issue 0、关闭 0 |
| 2026-09-12 | 匈牙利地形第 3 批当批核实（16 项，12 属实）（R44） | 7 | 0 / 7 / 0 | 当场修 7 条；新开 issue 0、关闭 0 |
...
| 2026-09-12 | travel · 机场距离条目内矛盾（2026-09-12 check:airports 第二段首跑捞出的一处（R34b）） | 1 | 0 / 1 / 0 | 当场修 1 条；新开 issue 0、关闭 0 |
...
| 2026-09-12 | travel · 越南中央直辖市的坊社数量、建制与排名（2026-09-12 越南 2025 行政改革核实：6 个中央直辖市 + 概览（R31b）） | 5 | 1 / 4 / 0 | 当场修 5 条；新开 issue 0、关闭 0 |
...
| 2026-09-11 | C1b 排名断言缺口径：141 处存量清理（R28a） | 152 | 0 / 152 / 0 | 当场修 152 条；新开 issue 0、关闭 0 |
| 2026-09-11 | 机场名核实·中国 A2 补核报出：义乌机场名写成了规划中新机场、牡丹江机场停航（R27d） | 4 | 0 / 3 / 1 | 当场修 3 条；新开 issue 1、关闭 0 |
| 2026-09-11 | 机场名核实·中国 A1 补核报出：嘉峪关机场已更名（R27c） | 3 | 0 / 3 / 0 | 当场修 3 条；新开 issue 0、关闭 0 |
...
| 2026-09-11 | 航线解说·国际 A（17 条：东亚 / 东南亚 / 中亚 / 大洋洲 / 北美 / 土耳其）（R23c） | 18 | 13 / 4 / 1 | 当场修 17 条；新开 issue 1、关闭 0 |
...
- **机场更名**没有任何自动防线（known-errors B1）。已知两例（乌鲁木齐、呼和浩特）
  都是人撞见才发现的，应当每隔一段时间把 944 座城市的机场名整体过一遍。
- **地形讲解仍是最大的一块未核实面**：1377 篇里虽然 1302 篇至少一个字段有留痕，
  但逐字段只有 1413 / 8262（17%）带核实留痕（issue #147）。
