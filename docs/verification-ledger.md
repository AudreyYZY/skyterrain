...
| 地形讲解 | 1362 × 6 段 × 2 语言 | **1041 个条目（全部，2026-09-11 实测）** | 计法：条目块内出现 `sources:` 留痕的算已核过，是**机械可复算**的口径，不是估计。**38 国全部整国核完一遍**（最后一批：哈萨克斯坦 / 克罗地亚 / 菲律宾，2026-09-11）。⚠️ 这是**覆盖率不是正确率** —— 早已收尾的冰岛后来又扫出同条目内自相矛盾；第二遍应按已知错型（「首座 / 唯一」、设立 vs 扩建年份、会过期的状态）全库回扫 |
