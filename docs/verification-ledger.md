...
| 地形讲解 | 1401 × 6 段 × 2 语言 | **1302 个条目至少一字段带核实留痕（2026-09-22 机械实测）** | `npm run gen:verify-status` 口径。⚠️ 这是**覆盖率不是正确率**，也不能把“条目有一字段核过”说成整条六段都核过；正确率仍只看均匀随机抽样。⚠️ **南非批 1/批 2/批 3 的「⚠️ 核实更正（日期）」措辞不匹配生成脚本的 `VERIFY_MARK` 正则**（该正则只认「核实 YYYY-MM」/「YYYY-MM-DD 核」/「级别 N」），导致这些条目在本表里被计成未核实——已提出为待修的工具 bug，见任务队列，不影响内容本身的核实状态（核实结论已落在 `docs/verification-ledger.md` 历史表与 `docs/africa-country-selection-2026-09-18.md`） |
