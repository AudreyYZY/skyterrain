...
lib/
  ...
  routes.ts              — 航线（data/routes/*.json，当前 279 条；航班号仅在有核实留痕时对外显示，见「航线的航班信息」一节），机场航点 + 地形航点
...
**前提**：本项目 1041 处地形 × 6 板块 + 691 座城市 × 7 段 + 279 条航线 × 2 套解说
...
- **写新内容时的硬性习惯**（这几条是从实际犯过的错倒推出来的，多数已由脚本强制）：
  ...
  ⑥-a **本项目有三套平行的内容系统，改一处要想到另外两处**：
     ...
     **航线解说（`route-narration.ts`，279 条 × 两套 × 中英）**。
...
**现状（2026-09-12 更新）**：279 条航线里 **274 条显示航班信息、5 条不显示**。
...
- **运动模型全部在 `lib/cesium/route-flight.ts`（纯函数，不依赖 Cesium/React）**，
  自检 `npm run check:flight` 对全部 279 条航线按 60 Hz 跑断言：进度映射单调、
...
- **解说锚定（学习模式）**：`lib/route-anchors.data.ts` 是逐句锚点表，
  ...
  **改了 `route-narration.ts` 的解说文字，必须重跑 `npm run gen:anchors`**，
  ...
  当前 558 篇（279 条 × 中英）里 528 篇已锚定，**待人工补 0 篇**；另 30 篇所属航线本来就没有
  地形/地标航点，无从锚定也不需要。
