...
**前提**：本项目 1041 处地形 × 6 板块 + 691 座城市 × 7 段 + 287 条航线 × 2 套解说
...
**现状**：287 条里 184 条已核实（161 条属实 + 23 条查明有误），103 条待核实。
已核完 15 国国内线：157 条里 125 条有误（**80%**）。另有 8 条现实中没有不经停航班的
航线已整条移除（不用「不显示航班号」把一条飞不了的线留在库里）。
国际线错误率约 26%（多为机型漂移），中国国内线抽查 24 条错了 19 条（多为航班号
根本不飞这条航线）—— 所以国内那批的默认假设应当是「未经核实即不可信」。
