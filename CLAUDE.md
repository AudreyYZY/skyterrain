...
- **已有**: 亚洲（中国 79 + 日本 26 + 韩国 28 + 蒙古 26 + 泰国 25 + 越南 25 + 马来西亚 26 +
  ...
  波兰 25 + 克罗地亚 26 + 捷克 26 + 匈牙利 26 = 575）+ 北美洲（美国 26 + 加拿大 25 = 51）+
  南美洲（智利 28 = 28）+ 大洋洲（澳大利亚 34 + 新西兰 30 = 64），
  中英双语，共 1067 个（39 个国家）
...
- **航线**：一条航线两套解说，跟随当前 `mode`（`getRouteNarration(id, lang, mode)`）；
  某模式解说为空时该模式下航线不播。当前 280 条航线 study + travel 解说均已写。
...
- **Phase B 逐国加地形，每国 ≥25 个 T1/T2/T3**：
  - ✅ 美国 26 · ✅ 加拿大 25（`north-america` 共 51）· ✅ 日本 26（`asia` 84→110）
    ...
    · 🚧 匈牙利（`europe` 549→，东欧次区域第三国，地形 26 条已完成；旅游模式首批 8 城）
  ...
  - 优先序（待做）：匈牙利其余批次；之后东欧候选斯洛伐克（见 expansion playbook）；南欧候选
    斯洛文尼亚 / 马耳他
...
**前提**：本项目 1041 处地形 × 6 板块 + 691 座城市 × 7 段 + 280 条航线 × 2 套解说
...
**现状（2026-09-12 更新）**：280 条航线里 **275 条显示航班信息、5 条不显示**。
...
- **运动模型全部在 `lib/cesium/route-flight.ts`（纯函数，不依赖 Cesium/React）**，
  自检 `npm run check:flight` 对全部 280 条航线按 60 Hz 跑断言：进度映射单调、
