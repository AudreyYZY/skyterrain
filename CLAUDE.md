...
lib/
  ...
  routes.ts              — 航线（data/routes/*.json，当前 284 条；航班号仅在有核实留痕时对外显示，见「航线的航班信息」一节），机场航点 + 地形航点
...
**地图/交互/双语已完成**：地形注册表（见 §范围，当前 1244 个）+ 数据驱动相机 +
...
- **航线**：一条航线两套解说，跟随当前 `mode`（`getRouteNarration(id, lang, mode)`）；
  某模式解说为空时该模式下航线不播。当前 284 条航线 study + travel 解说均已写。
...
- **`TERRAIN_REGISTRY`（当前 1244 个，见 §范围）= 单一真实源**：纯地貌图鉴，不含人文聚落
...
**前提**：本项目 1244 处地形 × 6 板块 + 768 座城市 × 7 段 + 284 条航线 × 2 套解说
...
- **运动模型全部在 `lib/cesium/route-flight.ts`（纯函数，不依赖 Cesium/React）**，
  自检 `npm run check:flight` 对全部 284 条航线按 60 Hz 跑断言：进度映射单调、
