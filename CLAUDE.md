...
- **`npm run check:zhen`** —— 比**中英两侧同一字段、同单位的数字**（地形 6 板块 + 城市 7 段，
  全库约 5,900 对）。中英是分开写的，人核实时又是**按语言**读的，所以「两侧各挑了一个值」
  ...
  ⚠️ **它每次都打印「实际比较了多少对」，0 对直接判失败** —— 这个脚本自己踩过两次
  「取不到值被当成没问题」（忘了 await 一个 async 函数、字段名写错成不存在的 `food`），
  两次都报了「全部通过」。**任何检查都要能说出自己实际检查了多少个对象。**
...
- **写新内容时的硬性习惯**（这几条是从实际犯过的错倒推出来的，多数已由脚本强制）：
  ...
  ⑥-a **本项目有三套平行的内容系统，改一处要想到另外两处**：
     ...
     **航线解说（`route-narration.ts`，280 条 × 两套 × 中英）**。
     ...
     —— 三处都是条目已改对、航线解说没跟上。
     **不花钱的第一步**：把航线解说里的最高级／排名，拿去和**本库已核过的地形条目**对，
     不需要联网。⚠️ 改完 `route-narration.ts` 必须重跑 `npm run gen:anchors`。
