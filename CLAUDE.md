...
- **已有**: 亚洲（中国 79 + 日本 26 + 韩国 28 + 蒙古 26 + 泰国 25 + 越南 25 + 马来西亚 26 +
  ...
  波兰 25 + 克罗地亚 26 + 捷克 26 + 斯洛伐克 28 + 匈牙利 26 + 斯洛文尼亚 28 + 马耳他 12 = 643）+ 北美洲（美国 26 + 加拿大 25 = 51）+
  南美洲（智利 28 = 28）+ 大洋洲（澳大利亚 34 + 新西兰 30 = 64），
  中英双语，共 1135 个（42 个国家）
...
lib/
  ...
  routes.ts              — 航线（data/routes/*.json，当前 281 条；航班号仅在有核实留痕时对外显示，见「航线的航班信息」一节），机场航点 + 地形航点
...
**地图/交互/双语已完成**：地形注册表（见 §范围，当前 1135 个）+ 数据驱动相机 +
...
- **航线**：一条航线两套解说，跟随当前 `mode`（`getRouteNarration(id, lang, mode)`）；
  某模式解说为空时该模式下航线不播。当前 281 条航线 study + travel 解说均已写。
...
- **`TERRAIN_REGISTRY`（当前 1135 个，见 §范围）= 单一真实源**：纯地貌图鉴，不含人文聚落
...
**前提**：本项目 1135 处地形 × 6 板块 + 734 座城市 × 7 段 + 281 条航线 × 2 套解说
...
- **运动模型全部在 `lib/cesium/route-flight.ts`（纯函数，不依赖 Cesium/React）**，
  自检 `npm run check:flight` 对全部 281 条航线按 60 Hz 跑断言：进度映射单调、
