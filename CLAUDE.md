...
- **已有**: 亚洲（中国 79 + 日本 26 + 韩国 28 + 蒙古 26 + 泰国 25 + 越南 25 + 马来西亚 26 +
  ...
  波兰 25 + 克罗地亚 26 + 捷克 26 + 斯洛伐克 28 + 匈牙利 26 + 斯洛文尼亚 28 + 马耳他 12 + 爱沙尼亚 26 + 拉脱维亚 27 + 罗马尼亚 29 = 725）+ 北美洲（美国 26 + 加拿大 25 = 51）+
  南美洲（智利 28 = 28）+ 大洋洲（澳大利亚 34 + 新西兰 30 = 64），
  中英双语，共 1217 个（45 个国家）
...
lib/
  ...
  routes.ts              — 航线（data/routes/*.json，当前 283 条；航班号仅在有核实留痕时对外显示，见「航线的航班信息」一节），机场航点 + 地形航点
...
**地图/交互/双语已完成**：地形注册表（见 §范围，当前 1217 个）+ 数据驱动相机 +
...
- **航线**：一条航线两套解说，跟随当前 `mode`（`getRouteNarration(id, lang, mode)`）；
  某模式解说为空时该模式下航线不播。当前 283 条航线 study + travel 解说均已写。
...
- **Phase B 逐国加地形，每国 ≥25 个 T1/T2/T3**：
  - ✅ 美国 26 · ✅ 加拿大 25（`north-america` 共 51）· ✅ 日本 26（`asia` 84→110）
    ...
    · ✅ 罗马尼亚 29（`europe` 696→725，东欧次区域；旅游模式 1 概览 + 8 城；航线待补 `pek-otp`）
  ...
  - **罗马尼亚**：`country: "romania"`（东欧次区域）；`COUNTRIES` 按拼音插在捷克与斯洛伐克之间（捷克 Jiékè < 罗马尼亚 Luómǎníyà < 斯洛伐克 Sīluòfákè）。
    ...
    **航线待补**：国航北京首都—布加勒斯特 2026-09-04 起每周三班、真不经停（之后续飞萨格勒布，所以克罗地亚一侧仍记「经停」），航班号 CA913/CA914 两个 3 级来源一致。
...
- **`TERRAIN_REGISTRY`（当前 1217 个，见 §范围）= 单一真实源**：纯地貌图鉴，不含人文聚落
...
**前提**：本项目 1217 处地形 × 6 板块 + 759 座城市 × 7 段 + 283 条航线 × 2 套解说
...
- **运动模型全部在 `lib/cesium/route-flight.ts`（纯函数，不依赖 Cesium/React）**，
  自检 `npm run check:flight` 对全部 283 条航线按 60 Hz 跑断言：进度映射单调、
