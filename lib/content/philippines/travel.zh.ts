import type { TravelGuide } from "@/lib/travel-lesson";

export const TRAVEL_ZH: Record<string, TravelGuide> = {
  // ============ 菲律宾 ============
  "philippines-overview": {
    // identity sources: PSA《2024 Census of Population (POPCEN) Population Counts Declared Official by the President》：2024-07-01 普查人口 112,729,484 人。https://psa.gov.ph/content/2024-census-population-popcen-population-counts-declared-official-president
    //   原文「逾 1.14 亿」方向就是错的——实际不到 1.14 亿。核实日 2026-09-07，来源级别 1
    identity:
      "菲律宾共和国是由约7107座岛屿组成的东南亚群岛国家，国土面积约29.98万平方公里，2024年人口普查为1.1273亿（菲律宾统计局），首都马尼拉，官方语言为菲律宾语（以他加禄语为基础）与英语，货币为菲律宾比索(PHP)。",
    layout:
      "国土大致分为三大岛群：吕宋岛（马尼拉、碧瑶、黎牙实比等，人口与经济核心区）、维萨亚斯群岛（宿务、保和、长滩岛等，中部岛屿群）与棉兰老岛（达沃等，南部岛屿）；巴拉望岛地处西部，自成一个相对独立的旅游区域。",
    gettingAround:
      "国内航班以马尼拉尼诺伊·阿基诺国际机场为枢纽，通达全国主要城市与岛屿；跨岛出行也可选择渡轮，部分离岛需转乘多段交通；市内出租车、吉普尼（改装小巴）与网约车Grab是常见代步方式。多国公民短期旅游的免签安排以菲律宾官方最新公布为准。",
    culture:
      "官方语言为菲律宾语与英语，英语普及率高、广泛用于商业与教育场合。菲律宾是东南亚天主教人口占比最高的国家，社会重视家庭观念与待客之道。参观天主教堂宜衣着得体，避免过短的短裤或无袖上衣。餐厅账单常已含服务费，另加10%左右小费较为常见。电压220V，插座多为美标两孔或欧标两圆脚。",
    seeAndDo:
      "自然地貌：马荣火山对称锥形、塔阿尔湖火山嵌套景观、巧克力山、普林塞萨港地下河。海岛度假：长滩岛白沙滩、艾尔尼多与科隆的喀斯特岛礁跳岛游。历史人文：维甘西班牙殖民老城、伊富高梯田。",
    whenAndTips:
      "地处热带，全年高温多湿；12月至次年5月为旱季（3-5月尤为炎热）、6月至11月为雨季，6至11月也是台风活跃季节，沿海与岛屿行程建议出行前查询台风预警。",
  },
  manila: {
    identity:
      "马尼拉是菲律宾首都，都会区人口逾1400万，坐落于吕宋岛马尼拉湾东岸，是菲律宾政治、经济、文化中心。",
    // howItWorks sources: Wikipedia (2026-09): Metro Manila (NCR 16 cities + Pateros, each elected mayor/council; MMDA; ~636 km²; ~14 million 2024; Greater Manila >26 million; City of Manila ~1.85 million; Quezon City ~2.96 million; Makati/BGC/Ortigas; EDSA; LRT/MRT); Local government in the Philippines
    howItWorks:
      "人们口中的马尼拉，是由 16 座市和 1 个镇拼合而成的国家首都区，官方称大马尼拉：每座市都有自己的民选市长和市议会，市之下再分 barangay，即有民选村长的基层社区；没有一个统一的都会政府，只有一个协调交通、洪水与垃圾的大马尼拉发展署。首都区约 636 平方公里，2024 年约 1,400 万人，而更大的都市连绵区超过 2,600 万。名义上的首都“马尼拉市”只是其中一座，约 185 万人，是西班牙时期的王城与老港所在；奎松市人口最多，约 296 万；商务中心则在马卡蒂、塔吉格的博尼法西奥环球城和帕西格与曼达卢永之间的奥提加斯，各在不同的市里。EDSA 大道是贯穿首都区的脊柱，轻轨与捷运沿几条干道铺开，吉普尼承担了大量出行。地址写“门牌、街道、barangay、市名”。初来的人最容易犯的错，是把“马尼拉市”当作整座城，而它只是十几座市里的一座。",
    layout:
      "王城区（马尼拉旧城）保留西班牙殖民时期城墙与教堂建筑，马卡蒂与博尼法西奥环球城是现代化商业金融区，马尼拉湾沿岸以日落景观闻名。",
    gettingAround:
      "尼诺伊·阿基诺国际机场距市区约11公里；市内轻轨与单轨覆盖主要商业区，出租车、吉普尼与网约车Grab是常见代步方式，市区交通高峰时段较为拥堵。",
    culture:
      "官方语言为菲律宾语与英语，英语普及率高。马尼拉融合西班牙殖民、美国托管与本土文化的多层历史印记，天主教信仰在社会生活中影响广泛。餐厅账单常已含服务费，另加10%左右小费较为常见。",
    seeAndDo:
      "王城区圣地亚哥堡与马尼拉大教堂；黎刹公园；马卡蒂现代商业区；马尼拉湾日落；国家博物馆群。",
    whenAndTips:
      "全年高温多湿，12月至次年2月气候相对凉爽干燥，是探访市区的较佳季节；6至11月为雨季与台风季，出行前建议查询天气。",
  },
  "cebu-city": {
    // identity sources: 菲律宾统计局（PSA）**2024 年人口普查（POPCEN，基准日 2024-07-01）**，已由总统第 973 号公告（2025-07-11）宣布为官方数 —— **这就是当前最新一期**。
    //   菲律宾人口只随普查更新：自 1970 年起每 5 年一次，2020 年是完整的人口与住房普查（CPH），2024 年是依第 87 号行政令做的期中人口普查；下一次完整普查目前指向 **2030 年**。
    //   口径提醒：**长滩岛不是一级行政区**，是马来镇下辖的 3 个 barangay，不能用马来镇全镇数（2024 年 64,723）代替；艾尔尼多、科隆本身就是一级市镇建制，用全镇数没问题。
    //   「宿务都会区」**不是 PSA 的法定区划**（不像国家首都区 NCR 有法律地位），只是本地惯用的非官方地理组合。
    //   宿务市（高度城市化市，独立于宿务省）2024 年普查 **965,332** 人，是中米沙鄢大区人口最多的高度城市化市。原文 identity 的「逾92万」既没年份、又与同条目 howItWorks 的 96 万对不上。
    identity:
      "宿务市是维萨亚斯群岛核心城市，2024年人口普查约96.5万，坐落于宿务岛东岸，是菲律宾历史最悠久的城市，也是中部群岛重要的经济与旅游枢纽。",
    // howItWorks sources: 菲律宾统计局（PSA）**2024 年人口普查（POPCEN，基准日 2024-07-01）**，已由总统第 973 号公告（2025-07-11）宣布为官方数 —— **这就是当前最新一期**。
    //   菲律宾人口只随普查更新：自 1970 年起每 5 年一次，2020 年是完整的人口与住房普查（CPH），2024 年是依第 87 号行政令做的期中人口普查；下一次完整普查目前指向 **2030 年**。
    //   口径提醒：**长滩岛不是一级行政区**，是马来镇下辖的 3 个 barangay，不能用马来镇全镇数（2024 年 64,723）代替；艾尔尼多、科隆本身就是一级市镇建制，用全镇数没问题。
    //   「宿务都会区」**不是 PSA 的法定区划**（不像国家首都区 NCR 有法律地位），只是本地惯用的非官方地理组合。
    //   「宿务都会区」不是 PSA 法定区划，只是本地惯用的非官方组合，已如实标注。
    howItWorks:
      "宿务市是一座“高度城市化的市”，在行政上独立于宿务省，不受省政府管辖，却仍是省会，省政府大楼在市内；市下分 80 个 barangay，各有民选的村长和村议会。2024 年人口普查约 96.5 万，而包括曼达维、拉普拉普、塔利赛等 13 个地方政府组成的「宿务都会区」（非官方组合，非 PSA 法定区划）约 320 万人，机场在拉普拉普市所属的麦克坦岛上。城市分两片：南面的老城在港口边，1565 年西班牙人在此建立菲律宾第一座城市，麦哲伦十字架、圣佩德罗堡与科隆街都在这里；北面的上城是宿务 IT 园区和阿亚拉中心，是新的商务与生活中心；山地的 barangay 占了市域的大半。宿务语是本地的日常语言。没有轨道交通，吉普尼与汽车承担出行。地址写“门牌、街道、barangay、市名”。初来的人最容易犯的错，是把宿务市与宿务岛、宿务省混同，而机场、度假区都在别的市。",
    layout:
      "老城区保留西班牙殖民时期教堂与要塞建筑，麦克坦岛（经跨海大桥相连）是国际机场与海滩度假区所在地，市中心IT商业园区是现代产业聚集地。",
    gettingAround:
      "麦克坦—宿务国际机场位于麦克坦岛，距市区约16公里，经跨海大桥可达；市内出租车与网约车Grab便捷可用，前往周边岛屿需转乘渡轮或快艇。",
    culture:
      "官方语言为菲律宾语与英语，当地宿务语（比萨扬语支）在日常生活中广泛使用。1521年麦哲伦船队在这里为宿务王公王后受洗、并赠予圣婴像作为洗礼礼物，这尊圣婴像此后延续供奉至今，是菲律宾现存最古老的天主教圣像（菲律宾第一台弥撒其实在南莱特省的利马萨瓦岛举行，宿务是紧随其后的受洗地）。餐厅账单常已含服务费。",
    seeAndDo:
      "麦哲伦十字架与圣婴大教堂；圣佩德罗堡；麦克坦岛海滩度假区；宿务姜黄咖喱（Lechon烤乳猪）等特色美食；周边岛屿跳岛游（近郊）。",
    whenAndTips:
      "12月至次年5月为旱季，是探访宿务及周边岛屿的较佳季节；6至11月为雨季与台风季，出行前建议查询天气。",
  },
  "davao-city": {
    // identity sources: 菲律宾统计局（PSA）**2024 年人口普查（POPCEN，基准日 2024-07-01）**，已由总统第 973 号公告（2025-07-11）宣布为官方数 —— **这就是当前最新一期**。
    //   菲律宾人口只随普查更新：自 1970 年起每 5 年一次，2020 年是完整的人口与住房普查（CPH），2024 年是依第 87 号行政令做的期中人口普查；下一次完整普查目前指向 **2030 年**。
    //   口径提醒：**长滩岛不是一级行政区**，是马来镇下辖的 3 个 barangay，不能用马来镇全镇数（2024 年 64,723）代替；艾尔尼多、科隆本身就是一级市镇建制，用全镇数没问题。
    //   「宿务都会区」**不是 PSA 的法定区划**（不像国家首都区 NCR 有法律地位），只是本地惯用的非官方地理组合。
    //   达沃市 2024 年普查 **1,848,947** 人（2020 年 1,776,949）。**原文的「逾190万」比任何一期普查都高** —— 2024 年实际数比它还低，方向就是错的。
    //   同条目「按行政区划面积计菲律宾陆地面积最大」复核后**仍成立**（2,443.61 km²，第二名普林塞萨港 2,381.02 km²）。
    identity:
      "达沃市是棉兰老岛最大城市，按行政区划面积计是菲律宾陆地面积最大的城市，2024年人口普查约184.9万，是棉兰老岛的政治经济中心，也是探索阿波山的核心门户。",
    // howItWorks sources: 「按面积计菲律宾最大的城市（2,443.61 km²）」与「2024 年普查 1,848,947、大马尼拉之外人口最多」**均核实属实**；面积第二是普林塞萨港 2,381.02 km²，与该条目的「第二大」互相自洽。2026-09-12 核。
    howItWorks:
      "达沃市是一座“高度城市化的市”，是达沃大区的中心，市域约 2,444 平方公里，是菲律宾面积最大的城市，但大部分是山地与农田，市区只占很小一角；市下分 3 个国会选区、11 个行政区与 182 个 barangay，各 barangay 有民选的村长和村议会。2024 年人口普查约 184.9 万，是马尼拉大都会以外人口最多的城市。市中心在波布拉西翁区，圣佩德罗大教堂与罗哈斯夜市在这里，拉南、马蒂纳、埃科兰是新的商业与住宅区。菲律宾最高峰阿波火山在市域西南，萨马尔岛隔着达沃湾在东面。宿务语是本地的日常语言。城市以严格的地方法规著称——禁烟、禁鞭炮、限速。榴莲是这座城的名片。没有轨道交通，出行靠吉普尼、三轮车与汽车。地址写“门牌、街道、barangay、市名”。初来的人最容易犯的错，是以为达沃是一座大城市，而市域的大部分是山。",
    layout:
      "市中心沿达沃湾分布，商业区与住宅区相对分散、覆盖广阔的行政区域，阿波山轮廓在市区东北方向清晰可见。",
    gettingAround:
      "弗朗西斯科·邦戈伊国际机场距市区约11公里；市内出租车、吉普尼与网约车Grab是主要代步方式，前往阿波山需公路自驾或包车约1.5至2小时。",
    culture:
      "官方语言为菲律宾语与英语，当地世居原住民族群与移民社区共同构成多元的达沃文化。达沃以城市治理有序、榴莲等热带水果盛产著称。餐厅账单常已含服务费。",
    seeAndDo:
      "阿波山国家公园徒步；达沃鳄鱼公园；人民公园；榴莲及热带水果品尝；埃甸海滩度假区（近郊）。",
    whenAndTips:
      "棉兰老岛地处台风带以南，全年气候相对稳定；12月至次年5月降水较少，是攀登阿波山的较佳季节，需提前预订许可与向导。",
  },
  baguio: {
    // identity sources: 菲律宾统计局（PSA）**2024 年人口普查（POPCEN，基准日 2024-07-01）**，已由总统第 973 号公告（2025-07-11）宣布为官方数 —— **这就是当前最新一期**。
    //   菲律宾人口只随普查更新：自 1970 年起每 5 年一次，2020 年是完整的人口与住房普查（CPH），2024 年是依第 87 号行政令做的期中人口普查；下一次完整普查目前指向 **2030 年**。
    //   口径提醒：**长滩岛不是一级行政区**，是马来镇下辖的 3 个 barangay，不能用马来镇全镇数（2024 年 64,723）代替；艾尔尼多、科隆本身就是一级市镇建制，用全镇数没问题。
    //   「宿务都会区」**不是 PSA 的法定区划**（不像国家首都区 NCR 有法律地位），只是本地惯用的非官方地理组合。
    //   碧瑶市 2024 年普查 **368,426** 人（2020 年 366,358，四年只增 2,068 人，年均 0.14%）。
    //   **「逾37万」这个措辞无论哪一期都站不住** —— 2020 与 2024 两期都不到 37 万。这不只是过期，是量级表述本身就错了。
    identity:
      "碧瑶是本格特省首府，2024年人口普查约36.8万，坐落于科迪勒拉山脉海拔约1500米的高原上，因凉爽气候被称为菲律宾“夏都”。",
    // howItWorks sources: 菲律宾统计局（PSA）**2024 年人口普查（POPCEN，基准日 2024-07-01）**，已由总统第 973 号公告（2025-07-11）宣布为官方数 —— **这就是当前最新一期**。
    //   菲律宾人口只随普查更新：自 1970 年起每 5 年一次，2020 年是完整的人口与住房普查（CPH），2024 年是依第 87 号行政令做的期中人口普查；下一次完整普查目前指向 **2030 年**。
    //   口径提醒：**长滩岛不是一级行政区**，是马来镇下辖的 3 个 barangay，不能用马来镇全镇数（2024 年 64,723）代替；艾尔尼多、科隆本身就是一级市镇建制，用全镇数没问题。
    //   「宿务都会区」**不是 PSA 的法定区划**（不像国家首都区 NCR 有法律地位），只是本地惯用的非官方地理组合。
    //   同上。
    howItWorks:
      "碧瑶是一座“高度城市化的市”，在行政上独立于本格特省——省会在隔壁的拉特立尼达——同时是科迪勒拉行政区的所在地；市下分 129 个 barangay，各有民选的村长和村议会。市域只有约 58 平方公里，2024 年人口普查约 36.8 万，建在海拔约 1,500 米的山上，凉爽的气候让它自 1903 年起成为菲律宾的“夏都”。城市按 1909 年设市时伯纳姆的规划建成：会议路是商业中心，伯纳姆公园是绿心，当年的设计人口只有 2.5 至 3 万人，如今的十几倍人口挤在同一片山坡上，拥堵是日常。伊巴洛伊人是这里的原住民，伊戈洛特各族、伊洛卡诺人、华人陆续迁入。几所大学让它成为一座大学城。上山靠肯农公路与马科斯公路，市内靠吉普尼与出租车。地址写“门牌、街道、barangay、市名”。初来的人最容易犯的错，是把碧瑶当作一座避暑小镇，而它是整个科迪勒拉地区的行政与商业中心。",
    layout:
      "市中心以碧瑶大教堂与彩虹公园为地标，环状道路沿山势起伏分布，是探索科迪勒拉山脉与普拉格山的重要门户城市。",
    gettingAround:
      "本地洛坎机场已停止商业客运，主要经马尼拉方向公路往返，山路车程约5至6小时；市内出租车与吉普尼是主要代步方式。",
    culture:
      "官方语言为菲律宾语与英语，本格特省原住民伊哥洛特族传统文化在当地保留完整。碧瑶因高原凉爽气候，20世纪初即被美国殖民当局开发为避暑胜地。餐厅账单常已含服务费。",
    seeAndDo:
      "碧瑶大教堂与彩虹公园；碧瑶市场手工艺品；普拉格山国家公园徒步（近郊）；松林公园骑马体验。",
    whenAndTips:
      "全年气候凉爽，12月至次年2月气温可降至10摄氏度左右，建议携带保暖衣物；前往普拉格山看云海建议凌晨出发。",
  },
  "puerto-princesa": {
    // identity sources: 菲律宾统计局（PSA）**2024 年人口普查（POPCEN，基准日 2024-07-01）**，已由总统第 973 号公告（2025-07-11）宣布为官方数 —— **这就是当前最新一期**。
    //   菲律宾人口只随普查更新：自 1970 年起每 5 年一次，2020 年是完整的人口与住房普查（CPH），2024 年是依第 87 号行政令做的期中人口普查；下一次完整普查目前指向 **2030 年**。
    //   口径提醒：**长滩岛不是一级行政区**，是马来镇下辖的 3 个 barangay，不能用马来镇全镇数（2024 年 64,723）代替；艾尔尼多、科隆本身就是一级市镇建制，用全镇数没问题。
    //   「宿务都会区」**不是 PSA 的法定区划**（不像国家首都区 NCR 有法律地位），只是本地惯用的非官方地理组合。
    //   普林塞萨港 2024 年普查 **316,384** 人（较 2020 年 307,079 增 0.72%）。同条目「按行政区划面积计菲律宾第二大城市」复核后**仍成立**（2,381.02 km²）。
    identity:
      "普林塞萨港是巴拉望省首府，2024年人口普查约31.6万，坐落于巴拉望岛中部，是探索普林塞萨港地下河与巴拉望岛自然景观的核心门户城市。",
    // howItWorks sources: 「菲律宾面积第二大的城市」与「地下河 1999 年列入世界遗产、在市区以北约 80 公里」**均核实属实**（UNESCO 官方给 76 公里，同一量级）。2026-09-12 核。
    howItWorks:
      "公主港 2007 年起是一座“高度城市化的市”，在行政上独立于巴拉望省，却仍是省会；市域约 2,381 平方公里，是菲律宾面积第二大的城市，绝大部分是森林与海岸，市下分 66 个 barangay，各有民选的村长和村议会。2024 年人口普查约 31.6 万，集中在公主港湾边的市区，机场就在市中心。1999 年列入世界遗产的地下河在市域内，但在市区以北约 80 公里；本田湾的跳岛在市区附近；爱妮岛在北面约 230 公里，属于另一个市镇。城市以环境治理著称，清洁与禁伐的规定是本地生活的一部分。三轮车是市内的主要交通。地址写“门牌、街道、barangay、市名”。初来的人最容易犯的错，是把公主港当作一个去地下河或爱妮岛的中转站，而它的市域本身就大到需要几小时车程。",
    layout:
      "市中心沿霍纳桑湾分布，是巴拉望岛行政与物资补给中心，周边分布多处石灰岩喀斯特地貌与热带雨林生态景观。",
    gettingAround:
      "普林塞萨港国际机场距市区约4公里；市内出租车与三轮摩托车（Tricycle）是主要代步方式，前往地下河国家公园需公路自驾或包车约1.5至2小时。",
    culture:
      "官方语言为菲律宾语与英语，本地保留有巴拉望原住民帕拉万族传统文化。普林塞萨港以生态旅游与自然保育闻名，是菲律宾知名的“生态之都”。餐厅账单常已含服务费。",
    seeAndDo:
      "普林塞萨港地下河国家公园（洞穴游船）；霍纳桑湾夜间萤火虎观赏；巴拉望野生动物园；地下河附近沙滩浮潜。",
    whenAndTips:
      "12月至次年5月为旱季，是探访地下河国家公园的较佳季节；地下河游船需提前在市区办理准证，建议提前一日预订。",
  },
  legazpi: {
    identity:
      "黎牙实比是阿尔拜省首府，人口逾21万，坐落于阿尔拜湾岸边、马荣火山南麓，是眺望马荣火山对称锥形景观的核心门户城市。",
    // howItWorks sources: en.wikipedia (2026-09, Tier B short): Legazpi (Philippines; component city and capital of Albay province and the regional centre of Bicol, 153.70 km² with 210,616 residents in the 2024 census, about 560 km south-east of Manila on Albay Gulf; south of Mayon Volcano, an active stratovolcano of near-symmetrical cone; originally the coastal settlement of Sawangan, a Spanish mission town from 1587, renamed in 1856 after Miguel López de Legazpi; served by Bicol International Airport in neighbouring Daraga; topped the National Competitiveness Council index for component cities in 2018)
    howItWorks:
      "黎牙实比是菲律宾阿尔拜省的省会，属“组成型城市”，即在省的管辖之下、由民选市长与市议会治理，同时是比科尔大区的行政中心；市域约 154 平方公里，2024 年人口约 21.1 万，在马尼拉东南约 560 公里的阿尔拜湾边。城市的一切都在马荣火山脚下展开：这座活火山以近乎完美的对称锥体著称，从市区抬头就能看见，也决定了这里的风险——熔岩与火山泥流的路径、疏散区的划分都按它来定，1814 年喷发掩埋的卡格沙瓦教堂只剩钟楼露在地面，如今是遗址公园。它原是名叫萨望甘的海边聚落，1587 年成为西班牙传教据点，1856 年改用远征队指挥官米格尔·洛佩斯·德·黎牙实比的名字。机场不在市内，而在相邻的达拉加，2021 年启用。台风季这一带首当其冲。初来的人最容易犯的错，是按晴天的照片来安排行程，而马荣多数日子被云挡住。",
    layout:
      "市中心沿阿尔拜湾分布，甲描育遗址等历史地标位于市郊，多个观景点可从不同角度眺望马荣火山完整锥体。",
    gettingAround:
      "比科尔国际机场距市区约8公里；市内出租车、三轮摩托车与吉普尼是主要代步方式，前往马荣火山周边景点需公路自驾或包车。",
    culture:
      "官方语言为菲律宾语与英语，当地比科尔语在日常生活中广泛使用。黎牙实比及周边比科尔地区以辛辣椰浆菜肴闻名，是菲律宾知名的美食产区。餐厅账单常已含服务费。",
    seeAndDo:
      "甲描育教堂遗址（火山喷发历史见证）；马荣火山国家公园周边观景点；卡格萨瓦废墟公园；达拉牙火山熔岩喷泉公园。",
    whenAndTips:
      "12月至次年5月为旱季，是眺望马荣火山与户外活动的较佳季节；6至11月为台风季，阿尔拜湾沿岸地区受台风影响相对明显，出行前建议查询预警。",
  },
  tagaytay: {
    // identity sources: 菲律宾统计局（PSA）**2024 年人口普查（POPCEN，基准日 2024-07-01）**，已由总统第 973 号公告（2025-07-11）宣布为官方数 —— **这就是当前最新一期**。
    //   菲律宾人口只随普查更新：自 1970 年起每 5 年一次，2020 年是完整的人口与住房普查（CPH），2024 年是依第 87 号行政令做的期中人口普查；下一次完整普查目前指向 **2030 年**。
    //   口径提醒：**长滩岛不是一级行政区**，是马来镇下辖的 3 个 barangay，不能用马来镇全镇数（2024 年 64,723）代替；艾尔尼多、科隆本身就是一级市镇建制，用全镇数没问题。
    //   「宿务都会区」**不是 PSA 的法定区划**（不像国家首都区 NCR 有法律地位），只是本地惯用的非官方地理组合。
    //   塔加泰 2024 年普查 **87,811** 人。**原文的「逾10万」不是过期，是与同条目 howItWorks 已有的 2024 年数直接矛盾** —— 8.8 万离 10 万差得远。
    identity:
      "塔加泰是甲米地省一座高原小城，2024年人口普查约8.8万，坐落于海拔约640米的高地，是眺望塔阿尔湖火山嵌套景观的知名观景城镇。",
    // howItWorks sources: en.wikipedia (2026-09, Tier B short): Tagaytay (Philippines; component city in Cavite province about 59 km from Manila via Aguinaldo Highway, at roughly 634 m on the Tagaytay Ridge; 87,811 residents in the 2024 census; the ridge looks across the caldera to Taal Volcano and Taal Lake, which lie in Batangas province, not in Tagaytay; average temperature about 22 °C, rarely above 31 °C; chartered as a city in 1938; called the second summer capital of the Philippines after Baguio; bulalo and tilapia from Taal Lake; Sky Ranch, People's Park in the Sky; agriculture in pineapples, coffee and cut flowers)
    howItWorks:
      "塔加泰是菲律宾甲米地省的一座“组成型城市”，即在省的管辖之下、由民选市长与市议会治理，2024 年人口约 8.8 万，在马尼拉以南约 59 公里。它建在一道海拔约 634 米的山脊上，气温常年在 22 度上下，很少超过 31 度，因此被称作菲律宾继碧瑶之后的“第二避暑之都”，周末马尼拉人上山，公路常年堵塞。山脊的另一侧是一个巨大的破火山口，口中蓄水成塔尔湖，湖中又有塔尔火山——这是它的标志性景观，但湖与火山都在南面的八打雁省境内，不属于这座城，塔加泰提供的是看它们的位置。2020 年 1 月塔尔火山喷发，火山灰落到马尼拉，湖区的村落疏散。本地饮食以牛骨汤“布拉洛”与塔尔湖的罗非鱼闻名，农业产菠萝、咖啡与鲜花。初来的人最容易犯的错，是以为火山在塔加泰境内，而这座城只是对岸的观景台。",
    layout:
      "城镇沿塔阿尔湖北岸山脊分布，多处观景台与餐厅面向塔阿尔湖开放，是马尼拉都会区居民周末近郊出游的热门目的地。",
    gettingAround:
      "本地无商业机场，经马尼拉方向公路往返，车程约1.5至2小时；镇内步行结合出租车是主要代步方式。",
    culture:
      "官方语言为菲律宾语与英语。塔加泰因高原凉爽气候与塔阿尔湖景观，自20世纪起即为马尼拉都会区居民偏爱的近郊度假地。餐厅账单常已含服务费。",
    seeAndDo:
      "塔阿尔湖观景台群；人民公园（People's Park in the Sky）；天空探险乐园高空滑索；本地菠萝与咖啡种植园参观。",
    whenAndTips:
      "全年气候较马尼拉凉爽，周末及节假日游客较多，建议避开高峰时段前往；塔阿尔火山偶有活动预警，出行前建议查询菲律宾火山地震研究所最新公告。",
  },
  boracay: {
    // identity sources: 菲律宾统计局（PSA）**2024 年人口普查（POPCEN，基准日 2024-07-01）**，已由总统第 973 号公告（2025-07-11）宣布为官方数 —— **这就是当前最新一期**。
    //   菲律宾人口只随普查更新：自 1970 年起每 5 年一次，2020 年是完整的人口与住房普查（CPH），2024 年是依第 87 号行政令做的期中人口普查；下一次完整普查目前指向 **2030 年**。
    //   口径提醒：**长滩岛不是一级行政区**，是马来镇下辖的 3 个 barangay，不能用马来镇全镇数（2024 年 64,723）代替；艾尔尼多、科隆本身就是一级市镇建制，用全镇数没问题。
    //   「宿务都会区」**不是 PSA 的法定区划**（不像国家首都区 NCR 有法律地位），只是本地惯用的非官方地理组合。
    //   **这条只能部分更新**：长滩岛不是一级行政区，是马来镇下辖的 3 个 barangay。2024 年普查已知 Manoc-Manoc **22,207**、Balabag **11,275**（两村合计 33,482），但 **Yapak 村的 2024 年数查不到**，全岛官方合计无法给出。
    //   按红线**不编一个合计数**：如实写明 2020 年基准 + 已知两村的 2024 年数 + 总数待公布。**也没有拿马来镇全镇 64,723 冒充全岛人口。**
    identity:
      "长滩岛是阿克兰省马来镇辖下一座知名度假岛屿，常住人口约3.8万（2020年人口普查；2024年普查最大的两个村已合计3.35万，全岛总数待官方公布），以白沙滩闻名，是菲律宾最知名的海岛度假目的地之一。",
    // howItWorks sources: 菲律宾统计局（PSA）**2024 年人口普查（POPCEN，基准日 2024-07-01）**，已由总统第 973 号公告（2025-07-11）宣布为官方数 —— **这就是当前最新一期**。
    //   菲律宾人口只随普查更新：自 1970 年起每 5 年一次，2020 年是完整的人口与住房普查（CPH），2024 年是依第 87 号行政令做的期中人口普查；下一次完整普查目前指向 **2030 年**。
    //   口径提醒：**长滩岛不是一级行政区**，是马来镇下辖的 3 个 barangay，不能用马来镇全镇数（2024 年 64,723）代替；艾尔尼多、科隆本身就是一级市镇建制，用全镇数没问题。
    //   「宿务都会区」**不是 PSA 的法定区划**（不像国家首都区 NCR 有法律地位），只是本地惯用的非官方地理组合。
    //   同上。
    howItWorks:
      "长滩岛是菲律宾阿克兰省的一座小岛，面积约 10.3 平方公里，2020 年人口普查约 3.8 万（2024 年普查两个最大的村已合计 3.35 万，全岛总数待官方公布）；岛不是一级行政区，而是分成三个“描笼涯”，归班乃岛上的马来镇管辖，因此这里的市政、税与执照都在对岸的镇政府。岛原是班乃布基农人与阿埃塔人的居住地。白沙滩长约四公里，沿岸按“一号、二号、三号站”分段，是全岛的主轴，酒店与餐厅都排在这条线上。2018 年 4 月起，为整治污水排放与环境破坏，政府下令全岛封闭半年，由跨部门工作组接管，同年 10 月重开，此后实行离岸 30 米的建筑退线与游客承载量上限——这套规则至今在执行，是理解这座岛现状的关键。岛上不通大型车辆，出行靠电动三轮与自行车；进岛经卡蒂克兰码头，机场在卡蒂克兰与卡利博。初来的人最容易犯的错，是以为岛上有机场，而航班落在对岸。",
    layout:
      "岛屿呈狭长哑铃形，白沙滩沿岛屿西岸绵延约4公里，分为一号、二号、三号站，岛屿东岸相对开发较少、保留自然海岸景观。",
    gettingAround:
      "戈多弗雷多·拉莫斯机场（俗称卡蒂克兰机场）位于长滩岛对岸马来镇，需转乘渡船约10分钟登岛；岛内以电动三轮车（E-trike）与步行为主要代步方式。",
    culture:
      "官方语言为菲律宾语与英语。长滩岛2018年曾因环境承载压力暂停开放6个月进行生态整治，此后限制沿岸开发密度，是菲律宾海岛旅游可持续管理的代表性案例。餐厅账单常已含服务费。",
    seeAndDo:
      "白沙滩日落；风筝冲浪（布拉波海滩）；跳岛游与浮潜；D'Mall购物餐饮区；岛屿周边潜水。",
    whenAndTips:
      "11月至次年4月为旱季，是海滩活动的最佳季节；6至10月受西南季风影响，东岸风浪较大、风筝冲浪爱好者反而偏爱此时前往布拉波海滩。",
  },
  "el-nido-town": {
    // identity sources: 菲律宾统计局（PSA）**2024 年人口普查（POPCEN，基准日 2024-07-01）**，已由总统第 973 号公告（2025-07-11）宣布为官方数 —— **这就是当前最新一期**。
    //   菲律宾人口只随普查更新：自 1970 年起每 5 年一次，2020 年是完整的人口与住房普查（CPH），2024 年是依第 87 号行政令做的期中人口普查；下一次完整普查目前指向 **2030 年**。
    //   口径提醒：**长滩岛不是一级行政区**，是马来镇下辖的 3 个 barangay，不能用马来镇全镇数（2024 年 64,723）代替；艾尔尼多、科隆本身就是一级市镇建制，用全镇数没问题。
    //   「宿务都会区」**不是 PSA 的法定区划**（不像国家首都区 NCR 有法律地位），只是本地惯用的非官方地理组合。
    //   艾尔尼多市镇 2024 年普查 **51,367** 人（全镇口径，该镇本身就是一级市镇建制）。**identity 停在旧数、howItWorks 已经是 2024 年普查数**，两段矛盾。
    identity:
      "艾尔尼多镇是巴拉望岛北端一座小镇，2024年人口普查约5.1万，是探索巴奎特群岛喀斯特岛礁景观的核心门户城镇。",
    // howItWorks sources: 「保护区 903 平方公里、菲律宾最大的海洋保护区」**核实属实**（903.21 km²）。2026-09-12 核。
    howItWorks:
      "艾尔尼多是菲律宾巴拉望省北端的一个市镇，由民选的镇长与议会治理，在省的管辖之下，2024 年人口约 5.1 万。镇区挤在一排石灰岩崖与海之间的窄地上，面朝布库特湾，湾里散着四十五座岛屿与礁石，最高的卡德劳岛达 640 米——这种喀斯特地貌是这里的全部卖点，也是行程的组织方式：镇上出发的跳岛游按 A、B、C、D 四条固定路线编排，由市镇统一管理。地名来自西班牙语的“巢”：崖壁上的金丝燕巢自古被采集，1954 年 6 月 17 日镇名由原来的巴库伊特改成艾尔尼多。整片海域属于艾尔尼多—泰泰资源管理保护区，面积 903 平方公里，是菲律宾最大的海洋保护区，进入需交环境费。普林塞萨港在南面约 269 公里，陆路要走五六个小时；本地有小型机场。初来的人最容易犯的错，是以为镇上就有沙滩，而好的海滩都在船程之外的岛上。",
    layout:
      "镇中心沿艾尔尼多湾分布，多家潜水店与跳岛游服务集中于海滨主街，镇区背靠陡峭的喀斯特山体。",
    gettingAround:
      "艾尔尼多机场为私营小型机场，航班选择有限，多数游客经普林塞萨港转乘公路前往（车程约5至6小时）；镇内跳岛游以租船或拼船形式为主。",
    culture:
      "官方语言为菲律宾语与英语。艾尔尼多以生态旅游发展著称，当地长期推行海洋保护与限流措施以维护巴奎特群岛的生态平衡。餐厅账单常已含服务费。",
    seeAndDo:
      "巴奎特群岛跳岛游（A、B、C、D四条经典航线）；大泻湖与小泻湖皮划艇；纳克帕克海滩徒步；艾尔尼多镇夜市。",
    whenAndTips:
      "11月至次年5月为旱季，是跳岛游的最佳季节；6至10月受季风影响部分航线可能取消，出行前建议与当地跳岛游服务商确认。",
  },
  "coron-town": {
    // identity sources: 菲律宾统计局（PSA）**2024 年人口普查（POPCEN，基准日 2024-07-01）**，已由总统第 973 号公告（2025-07-11）宣布为官方数 —— **这就是当前最新一期**。
    //   菲律宾人口只随普查更新：自 1970 年起每 5 年一次，2020 年是完整的人口与住房普查（CPH），2024 年是依第 87 号行政令做的期中人口普查；下一次完整普查目前指向 **2030 年**。
    //   口径提醒：**长滩岛不是一级行政区**，是马来镇下辖的 3 个 barangay，不能用马来镇全镇数（2024 年 64,723）代替；艾尔尼多、科隆本身就是一级市镇建制，用全镇数没问题。
    //   「宿务都会区」**不是 PSA 的法定区划**（不像国家首都区 NCR 有法律地位），只是本地惯用的非官方地理组合。
    //   科隆市镇 2024 年普查 **69,439** 人（16,483 户）。同上，两段矛盾。
    identity:
      "科隆镇是巴拉望岛东北部布桑加岛上一座小镇，2024年人口普查约6.9万，是探索科隆—卡拉棉群岛石灰岩地貌与二战沉船潜水的核心门户城镇。",
    // howItWorks sources: en.wikipedia (2026-09, Tier B short): Coron (Philippines; municipality covering the eastern part of Busuanga Island, Coron Island and about 50 islets in northern Palawan, 689.10 km² with about 69,439 residents in 2024; the town proper is on Busuanga while Coron Island lies opposite; originally Bancuang, renamed by the Spanish, a municipality from 2 June 1902; Japanese occupation May 1942 to March 1945; about a dozen sunken Japanese warships from the aerial raid of September 1944 lie at 10–40 m and are the main dive sites; Siete Pecados Marine Park, 2024 Blue Park Award; Francisco B. Reyes Airport with flights to Manila, Cebu and Angeles City)
    howItWorks:
      "科隆镇是菲律宾巴拉望省北部的一个市镇，由民选的镇长与议会治理，辖布桑加岛的东半部、科隆岛以及约五十座小岛，总面积约 689 平方公里，2024 年人口约 6.9 万。这里有一个常被弄混的关系：镇子在布桑加岛上，而对面那座峭壁林立、有卡扬根湖与梭鱼湖的科隆岛是另一座岛，属于原住民塔格班瓦人的祖传领地，出入受他们管理，只能按规定的路线与时间进入。镇名原为班庫昂，西班牙人改成今天的写法，1902 年 6 月 2 日设镇。1944 年 9 月美军空袭击沉了停泊在这片海域的日本船只，十多艘沉船躺在水下十到四十米，成为世界知名的沉船潜点，与镇外的温泉一起支撑着旅游业；七罪岛海洋公园 2024 年获得蓝色公园奖。机场在布桑加岛，有飞往马尼拉与宿务的航班。初来的人最容易犯的错，是把科隆镇与科隆岛当成一个地方，而它们隔着一道海。",
    layout:
      "镇中心沿科隆湾分布，是布桑加岛及周边卡拉棉群岛的行政与物资补给中心，通往卡杨甘湖等岛内景点需转乘公路或渡船。",
    gettingAround:
      "弗朗西斯科·雷耶斯机场距镇区约30公里，需转乘公路或渡船；镇内跳岛游以租船或拼船形式为主。",
    culture:
      "官方语言为菲律宾语与英语，本地塔格巴努亚族原住民对科隆群岛内多处自然景观（包括卡杨甘湖）拥有传统管理权。餐厅账单常已含服务费。",
    seeAndDo:
      "卡杨甘湖游览；二战日本沉船潜水；双子泻湖；卡扬甘岛与巴拉库达湖跳岛游；科隆镇温泉。",
    whenAndTips:
      "11月至次年5月为旱季，是跳岛游与沉船潜水的最佳季节；卡杨甘湖等景点由塔格巴努亚族社区管理，入园需缴纳环境保护费。",
  },
  tagbilaran: {
    // identity sources: 人口：菲律宾统计局 2024 年人口普查 106,120。口径是**普查人口**，菲律宾市级人口只随普查更新，停在 2024 年不算过期。
    identity:
      "塔比拉兰是保和省首府，2024年普查人口约10.6万（菲律宾统计局），坐落于保和岛西南岸，是探索巧克力山与保和岛自然景观的核心门户城市。",
    // howItWorks sources: 「组成型城市、民选市长与市议会」与「全省人口最多」**均核实属实**（2024 年普查 106,120，第二为乌拜 82,179）。2026-09-12 核。
    howItWorks:
      "塔比拉兰是菲律宾保和省的省会，属“组成型城市”，即在省的管辖之下、由民选市长与市议会治理，市域约 36.5 平方公里，2024 年人口约 10.6 万，是全省人口最多的地方。它的角色是门户而不是目的地：省内著名的巧克力山在内陆的卡门镇，眼镜猴保护区在科雷利亚，都要从这里出发走一到两小时；邦劳岛在南面，一条堤道相连，2018 年启用的保和—邦劳国际机场就在那座岛上，海滩与潜点也在那边。城市 1742 年设镇，1966 年设市；1565 年米格尔·洛佩斯·德·黎牙实比与本地首领西卡图纳在这一带以血为盟，是菲律宾史上常被引用的一幕，纪念碑立在城郊。宿务在北面，渡轮约两小时。2013 年 7.2 级地震损毁了保和岛多座西班牙时期的石造教堂，修复工作持续多年。初来的人最容易犯的错，是以为机场在市内，而它在跨过堤道的邦劳岛上。",
    layout:
      "市中心沿保和海峡分布，是保和岛行政与交通枢纽，前往巧克力山与内陆眼镜猴保护区需向岛屿中部行进。",
    gettingAround:
      "保和—邦劳国际机场2018年启用，位于邦劳岛，距市区约18公里，取代原塔比拉兰机场；市内出租车、三轮摩托车与网约车Grab是主要代步方式。",
    culture:
      "官方语言为菲律宾语与英语，当地比萨扬语支的保和语在日常生活中广泛使用。保和岛以眼镜猴保护与巧克力山地质景观闻名，是维萨亚斯群岛重要的生态旅游目的地。餐厅账单常已含服务费。",
    seeAndDo:
      "巧克力山观景台；眼镜猴保护中心；洛博克河游船；巴克拉容教堂遗址；邦劳岛海滩（近郊）。",
    whenAndTips:
      "12月至次年5月为旱季，是探访巧克力山与岛内景点的较佳季节；巧克力山观景台清晨光线较柔和，适合安排早间行程。",
  },
  vigan: {
    identity:
      "维甘是伊罗戈斯苏尔省首府，人口逾5.4万，坐落于吕宋岛西北海岸，是菲律宾保存最完整的西班牙殖民时期老城，1999年列入UNESCO世界遗产名录。",
    // howItWorks sources: 「1999 年列入世界遗产」与「2015 年入选新七大奇迹城市」**均核实属实**（UNESCO 官方案例页 + New7Wonders 官网，1 级）。2026-09-12 核。
    howItWorks:
      "维甘是菲律宾南伊罗戈省的省会，属“组成型城市”，即在省的管辖之下、由民选市长与市议会治理，2024 年人口约 5.4 万，在吕宋岛西岸、马尼拉以北约 404 公里。1999 年“维甘历史城镇”列入世界遗产，理由是它是亚洲保存最完好的西班牙殖民城镇范例之一，2015 年又入选“新七大奇迹城市”。它保存下来的原因在于结构：老城以萨尔塞多广场与布尔戈斯广场为中心，四周是“混血区”，那是华人与西班牙人通婚的商人家族聚居之处，他们的宅子叫“石屋”——下层石砌、上层木造、窗格镶贝壳，把闽南、西班牙与热带气候的做法合在一起。克里索洛戈街的石块路面与两侧连排的老宅是这一片的核心，马车至今在街上载客。帕格布尔纳扬的陶窑与手织布仍在生产，维甘香肠是本地食物。机场在北面 80 公里的拉瓦格。初来的人最容易犯的错，是把老城当作复原的布景，而这些宅子多为原物，至今有人住。",
    layout:
      "老城核心区街道呈棋盘式布局，两侧保留大量西班牙殖民时期石砌建筑与骑楼商铺，横甘街是老城最具代表性的历史街区。",
    gettingAround:
      "本地维甘机场客运航班暂不稳定，多数游客经马尼拉方向公路往返，车程约7至8小时；老城核心区步行可达，马车（Kalesa）是当地特色代步工具。",
    culture:
      "官方语言为菲律宾语与英语，当地伊洛卡诺语在日常生活中广泛使用。维甘是菲律宾、中国与西班牙文化长期交融的历史见证，老城建筑风格融合欧洲与华南骑楼元素。餐厅账单常已含服务费。",
    seeAndDo:
      "横甘街石板路老城区；维甘大教堂；西索克南遗产村；马车观光游；维甘朗西隆（当地特色香肠）品尝。",
    whenAndTips:
      "11月至次年2月气候相对凉爽干燥，是探访老城的较佳季节；老城核心区建议安排一整天步行结合马车游览。",
  },
};
