...
export const TRAVEL_EN: Record<string, TravelGuide> = {
  ...
  "newcastle-upon-tyne": {
    ...
    gettingAround:
      "Newcastle International Airport (NCL), about 12 km from the centre, is linked directly by the Tyne and Wear Metro (about 25 minutes), with scheduled flights. Trains from London King's Cross take about 3 hours and from Edinburgh about 1.5. The city has the Metro and buses; the centre is walkable and the Quayside is a riverside stroll; take a bus to the Angel of the North and a train to Hexham or the summer AD122 bus for Hadrian's Wall. The climate is temperate maritime, cool and breezy.",
    culture:
  ...
  portree: {
    ...
    howItWorks:
      "Portree is the largest settlement on Skye, with about 2,300 people in 2022, the only place on the island that can be called a town, but it has no council of its own: Skye belongs to the Highland Council based in Inverness, whose local office stands in town, while health and police belong to the Scottish Government. The town wraps around a small harbour with painted houses along the quay and is the island's centre for shops, hospital and buses; the Skye Bridge lies 55 km south-east at Kyleakin, the Old Man of Storr, the Quiraing and the Fairy Pools are scattered across the island, which runs about 80 km long, buses are sparse and about four in ten residents speak Gaelic. Newcomers most often go wrong by taking Portree for Skye, when the island's sights lie tens of kilometres away and nothing moves without a car.",
    layout:
