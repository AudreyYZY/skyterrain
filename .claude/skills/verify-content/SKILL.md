...
**红线指令要写判据，不要写黑名单。** 2026-09-10 同一轮里栽了两次：给子代理的 prompt 里
写了「突出部之役一律不写」「核试验场这一整块一律不写」，它就照做，还注明「依据是任务指令」——
**拿回来的是我自己的话被复述了一遍，不是核实**。正确的写法是给判据：
「涉及一个**仍在持续的对立状态**、或带**评价性表述**的，删；只是定位性的简短事实，留」，
它才有可能报回「这一条不该删」。（判例：意大利一战山地战、越南的挂牌景点、阿登战役 —— 都保留；
琉球「规模最大的地面战」、谢梅伊「承受深重的核辐射影响」—— 都改，因为前者是最高级排名、
后者是评价词。）

**两个 1 级来源冲突时，先看它们是不是「对称」的 —— 有一个解释了另一个为什么错，就不要并列。**（2026-09-18 立）
本库的既定做法是「两说并存就并列、不选边」，但那条规则**默认两个来源地位相当**。纳米比亚这一轮撞到一个不对称的例子：
**索苏斯维来的沙丘是不是「世界最高」——环境部（MEFT）的旅游页至今写着「Highest sand dunes in the world」，
而同一国政府署名的 UNESCO 申报文本明说它们「**不是**绝对高度上最高的」**，还列表对比了秘鲁 Cerro Blanco（1,176 m）
与巴丹吉林（500 m），并写明跨区域的沙丘高度比较「不出现在地貌学文献里」。
两个都是 1 级、都是同一国政府，但**一个是营销文案、一个是专业申报文件，而且后者直接解释了前者为什么不成立**。
这时候并列两说等于把一句已知错误的话留在正文里。
判据：**问「这两个来源各自是为什么写的、谁在这个问题上是专业的、有没有一方解释了另一方的成因」**。
解释得了的那一方胜出；解释不了、只是各说各话的，才并列。
（同一轮还查出：流传最广的「纳米布沙漠有 5,500 万年历史」**在 1–2 级来源里一次都没出现**，
而查到的五个年代数各自指的是五件不同的事 —— **一个流传很广的数字查不到任何像样出处，本身就是结论**。）

**查「有没有 X」时，要确认自己查的那张表回答的是不是这个问题。**（2026-09-18 立，冰岛批当批栽的）
写邵扎克罗库尔的交通时，我查了 Strætó 的乡村线路总表，表里**没有一条线路的起点或终点是这座镇**，
于是写下「全国乡村公交线网里没有到本镇的线路」。核实查出来是错的：**干线 57 路确实在该镇设站** ——
我看的是「线路一览」（只列起终点），而要回答的问题是「有没有车停这里」，那要看**沿途站点**或票价区表。
**「表里没列出来」不等于「不存在」，多数时候只说明这张表不回答你的问题。**
同型的坑：机场「目的地」页不回答「现在有没有航司在飞」（那要看运营方的当前航司清单）；
行政区划表不回答「这个地名今天属于谁」（那要看最近一次合并 / 改名的公告）。
判据：**写「没有 / 查无」之前，先说出自己查的那张表的口径是什么，以及它为什么能回答这个问题。**
说不出来就说明还没查到位 —— 这类错在正文里读起来完全正常，任何脚本都看不见，只有核实能抓。

**写下或核对任何「唯一 / 最……」之前，先拿那个属性词在本库同国家的条目里搜一遍。**
排他性断言是**一次对全库的断言**，而全库就在手边、不需要联网。已经栽过四次：
阿拉拉特的「唯一有常年冰帽的孤立火山」被本库的 `mount-suphan` 证伪；
格伦维的「爱尔兰面积最大的国家公园」被本库的 `wicklow-mountains` 证伪；
雷克雅内斯的「全世界唯一能在陆地上看到大洋中脊的一段」**被它自己条目的另外两段**证伪。
**「首个 / 第一个」触发同一条判据** —— 塞穆瓦河谷的「瓦隆大区首个自然公园」实际是同日生效的两座之一。

**写「A 是 B 造成的」这类成因断言之前，先拿这个对象的专名在本库里搜一遍。**
和上一条是同一个动作，只是搜的东西从属性词换成了专名。已经栽过两次：航线解说与地形条目
对同一片沙漠各说各的；`teide` 与 `caldera-de-taburiente` 对**同一座**火山口，一个写侧向崩塌、
一个写岩浆房塌陷 —— 而那本来就是学界未定论的问题。
**查下来是学界未定论时，本库的写法只有一种：并列两说 + 写明有争论。**
不要因为「这一段需要一个成因」就选一边 —— 一旦选了，同批和后续条目会被带着一起错。

**修完一条事实，拿那个「错的值」在全库 grep 一遍再收工。**
**这一步现在由 `verify:apply-text` 自动做了**（打完补丁会列出「刚改掉的说法在别处还留着几处」），
但**判断仍然归你**：命中不等于有错（「安特卫普港」在别处出现 8 次都是正常的地理指代）。
逐条看一遍，同一件事就一起改。
不是拿正确值搜 —— 正确值搜出来的都是对的，**错的值搜出来的才是漏网的**。
同一天里栽过两次：改了 `oland-alvar` 的「石炭纪」，`gotland` 里的交叉引用没跟着改；
改了 `bardenas-reales` 的「阿拉贡王室」，`ebro-basin` 里还写着阿拉贡。
照做一次就又捞出「梅尔梅奥河谷」这个假地名的另外三个落点 —— 它一共住在四个地方。
**`terrain-registry.ts` 的 `source` 字段也要一起搜**：它是留痕不是正文，但下一个人照样会拿它当依据。

**`history` 段要和数字一视同仁地核 —— 故事越顺，越要查。**
本库抓到过一整段被换掉的历史：雷波韦西写成「苏联边境军事管制区、长期禁止公众进入、
2003 年解禁建园」，实际是造纸企业 2001 年捐地促成建园，而且建园前几十年当地就是徒步热门地。
「冷战边境 → 军事管制 → 禁入 → 解禁」是一条太顺的叙事线，顺到没人会去查。
核 `history` 时去查该国主管机构（国家公园管理局、环境部）的建园/设立经过原文。

**核「X 年获评 / 列入 Y」时，先确认 Y 这套制度在该国有几个案例、本条对应哪一个。**
同一天栽过两次：芬兰 `salpausselka-ridge` 用了罗库阿（该国**首个**地质公园，2010）的年份，
...
**专名要单独拿去搜一遍，搜不到就删，不要因为上下文合理放过去。**
数字有 `C6` 族、最高级有 `C1a`/`C1b`、拼接有 `D4`，**一个编出来的地名什么都不触发**。
`torne-river` 中文写的 UNESCO 遗产名「梅尔梅奥河谷」不对应任何真实遗产，
而同一句的英文写的是正确的「Struve Geodetic Arc」—— 两侧各写各的，`check:zhen` 也比不出来。

**「注释里有年份」不等于「正文有年份」。** `check:claims` 的 `C6` 量的是正文 ——
读者和 TTS 读到的就是正文。核人口数字时，除了确认年份/口径/出处，
还要**明说正文本身缺不缺**，并给出把年份与口径写进正文的替换文本。
韩捷 12 城那一轮就是全部判 ok、而 `C6` 一处没降，因为年份都只在注释里。
