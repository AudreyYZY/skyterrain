...
**核实方也会错 —— 收到 wrong 判定时，先看它自身有没有自相矛盾。**（2026-09-18 立，一天之内撞到两次）
这个流程的默认假设是「核实方说错了就是错了」，但实测**核实方的 wrong 判定本身也会错**，而且两次都有明显的「破绽」：

1. **爱尔兰批**：核实方判都柏林机场的「≥1mm 降水日 137.1」口径标错，说该表只有 ≥0.2mm（199.6）、
   **≥0.1mm（137.1）**、≥5.0mm（48.8）三档。**破绽是物理上不可能**：阈值越低、达标天数只会越多，
   而它给的 ≥0.1mm（137.1）竟然小于 ≥0.2mm（199.6）。回原表一查，那一行就是 **≥1.0mm = 137.1**，
   核实方凭空造了一个不存在的「≥0.1mm」行。**原文是对的，按它改反而会改错。**
2. **抽样 seed=19**：核实方给埃施瑟尔到机场「直线约 55 公里」，而按本库两端坐标实算是 38.6 公里 ——
   它引的是第三方旅行站点的**驾车**距离，标签写成了直线。

**判据**：收到 wrong 之后，动手改之前先问三件事 ——
① **它给的数与它自己给的其他数自洽吗**（单调性、量纲、加总）？
② **它引的来源级别是什么**？3 级站点的「距离」默认是公路里程、「时长」默认是聚合估算。
③ **我原来的来源是不是比它的更硬**？（爱尔兰的铁路时长，我方用的是官方 PDF 时刻表里四趟具体车次，
   核实方用的是 rome2rio 一类聚合站 —— 这种情况下**不要因为它说「来源较弱」就去改**。）
三条都过不去再改。**核实的价值在于它诚实，不在于它永远对。**
