...
| 模式 | 面向的问题 | 内容 |
|---|---|---|
| **学习模式** | "这片是什么地形？怎么形成的？" | 地形地貌图鉴 —— 1402 处地貌 + 6 板块讲解 |
...
- 学习模式 —— 亚洲 · 欧洲 · 非洲 · 北美洲 · 南美洲 · 大洋洲共 53 个国家，1402 处地貌，中英双语
- 旅游模式 —— 覆盖学习模式已上线国家中的 53 个，共 53 个国家概览 + 959 座城市，中英双语；
  300 条航线（国内 + 国际，每条附机场/地形航点 + 学习/旅游两套双语解说）——
  **其中 286 条有经核实后可展示的航班信息**，见下面「数据可靠性」

**规划**：继续逐国扩展全球地形覆盖 + 按旅游热度扩展各国旅游模式覆盖城市。
...
- **1402 处地貌**统一注册在 [`lib/terrain-registry.ts`](lib/terrain-registry.ts)，14 类：
...
- 几何自检：`node --experimental-strip-types scripts/check-terrain-camera.ts`（1402/1402）。
...
- **1402 处全部有中英双语讲解**。写作口径：去比较性 / 主观评价，有争议的加限定或并列，
...
- **300 条航线**（`data/routes/*.json`，含国内航线与
  中国—他国国际航线），每条 = 一对真实通航的城市 + 出发到达机场 + 沿途地形航点，
  数据里在核实得到时另存航司 / 航班号 / 机型。
  **不要把航班字段当成实时航班表**：它们只是 `checkedOn` 当天的快照；无法可靠核实的航线
  只保留地理路线，不展示航班号、机型或时长。
...
- **航班号/机型只有核实过才对外显示**：`FlightRoute.source`（`ref` / `checkedOn` /
  ...
  或航班号"。
- **现状：300 条均有机器可读的来源状态**——286 条可展示核实后的航班信息；
  5 条已查明原信息有误、9 条只能确认航点关系，后 14 条不显示航班号或机型。
  2026-09-07 的首轮 280 条审计结果是 91 条属实、163 条有误已改正、26 条有误且找不到可靠替代。
  **逐条核下来 189 条至少有一处错，占 68%**；冰岛、挪威、法国、意大利、西班牙、
  葡萄牙、丹麦、蒙古八国的国内线错误率是 100%。另有 15 条现实中根本没有不经停航班
  的航线已整条移除（295 → 280）。
  错法以"这个航班号根本不飞这条航线"和机型漂移为主。**即便是核对属实的那些，也只是
  `checkedOn` 那天的快照**，航司改期换机不会自动同步——出行请以航司官网或 OTA 的
  实时信息为准。
...
这个项目的文字量很大（1402 处地形 × 6 板块 + 959 座城市 × 7 段 + 300 条航线 × 2 套解说，
...
| 航线的**航班号 / 机型** | 中（有来源状态） | 283 条可展示核实信息；5 条 `wrong` + 9 条 `flight-unknown` 不显示航班号或机型 |
...
npm run check:camera   # 相机几何（1402/1402）
...
npm run check:camera    # 相机几何自检（1402/1402）
...
lib/
  terrain-registry.ts       — 【单一真实源】1402 处地形的锚点/范围/走向/中英名/来源
  ...
  routes.ts / route-narration.ts — 航线（当前 300 条，国内+国际；航班号仅在核实过时对外显示）
                                    + 每条学习/旅游两套连贯解说
...
data/
  *.json                    — 早期地形数据（坐标现由 terrain-registry 覆盖）
  routes/                   — 航线定义（当前 300 条，国内+国际，含 source 核实留痕）
...
| 航班号 / 机型 | 300 条均有来源状态；286 条可展示，5 条 `wrong` + 9 条 `flight-unknown` 不对外显示；**其余也只是核实当日的快照**，航司改期换机不会自动同步 |
