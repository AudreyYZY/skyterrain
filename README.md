...
- 旅游模式 —— 覆盖学习模式已上线国家中的 53 个，共 53 个国家概览 + 959 座城市，中英双语；
  303 条航线（国内 + 国际，每条附机场/地形航点 + 学习/旅游两套双语解说）——
  **其中 289 条有经核实后可展示的航班信息**，见下面「数据可靠性」
...
- **303 条航线**（`data/routes/*.json`，含国内航线与
...
- **航班号/机型只有核实过才对外显示**：`FlightRoute.source`（`ref` / `checkedOn` /
  ...
  或航班号"。
- **现状：303 条均有机器可读的来源状态**——289 条可展示核实后的航班信息；
...
这个项目的文字量很大（1402 处地形 × 6 板块 + 959 座城市 × 7 段 + 303 条航线 × 2 套解说，
...
| 航线的**航班号 / 机型** | 中（有来源状态） | 289 条可展示核实信息；5 条 `wrong` + 9 条 `flight-unknown` 不显示航班号或机型 |
...
lib/
  ...
  routes.ts / route-narration.ts — 航线（当前 303 条，国内+国际；航班号仅在核实过时对外显示）
                                    + 每条学习/旅游两套连贯解说
...
data/
  *.json                    — 早期地形数据（坐标现由 terrain-registry 覆盖）
  routes/                   — 航线定义（当前 303 条，国内+国际，含 source 核实留痕）
...
| 航班号 / 机型 | 303 条均有来源状态；289 条可展示，5 条 `wrong` + 9 条 `flight-unknown` 不对外显示；**其余也只是核实当日的快照**，航司改期换机不会自动同步 |
