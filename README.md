...
| 模式 | 面向的问题 | 内容 |
|---|---|---|
| **学习模式** | "这片是什么地形？怎么形成的？" | 地形地貌图鉴 —— 106 处地貌 + 6 板块权威讲解 |
| **旅游出行模式** | "到了这个陌生城市，衣食住行要注意什么？" | 城市概览 —— 地理格局 / 衣食住行 / 人文习俗 / 出行提示 |

两套内容系统平行、互不影响。

**当前范围**：
- 学习模式 —— 中国全境（84，含新疆全境）+ 澳大利亚（22），中英双语，逐句核源
- 旅游模式 —— 澳大利亚（国家概览 + 7 城市），中英双语
...
- **106 处地貌**统一注册在 [`lib/terrain-registry.ts`](lib/terrain-registry.ts)，15 类：
...
- 几何自检：`node --experimental-strip-types scripts/check-terrain-camera.ts`（106/106）。
...
- **106 处全部有讲解**，其中 61 篇（中国 34 + 澳大利亚 22 + 新疆绿洲聚落 5）已逐句核源：
  去比较性 / 主观评价，有争议的加限定或并列，查不到宁可删，数字统一到权威口径。
...
node --experimental-strip-types scripts/check-terrain-camera.ts   # 相机几何自检（106/106）
...
lib/
  terrain-registry.ts       — 【单一真实源】106 处地形的锚点/范围/走向/中英名/来源
