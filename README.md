...
- 旅游模式 —— 覆盖学习模式已上线的全部 38 个国家，共 38 个国家概览 + 691 座城市，中英双语；
  295 条航线（国内 + 国际，每条附机场/地形航点 + 学习/旅游两套双语解说）——
  **其中只有 59 条的航班号/机型经过核实**，见下面「数据可靠性」
...
- **295 条航线**（`data/routes/*.json`，覆盖上面列出的所有国家，含国内航线与
  中国—他国国际航线），每条 = 一对真实通航的城市 + 出发到达机场 + 沿途地形航点，
  数据里另存有航司 / 航班号 / 机型。
  **不要把这 295 条整体当成"真实商业航班"**：城市对与机场是可查的，但写入时给出的
  航班号/机型是当时的一个示例，抽查下来相当一部分对不上（中国国内线抽查 24 条、
  19 条的航班号根本不飞这条航线）。
...
- **航班号/机型只有核实过才对外显示**：`FlightRoute.source`（`ref` / `checkedOn` /
  ...
  或航班号"。
- 现状：295 条里 **59 条已核实**（45 条属实 + 14 条查明有误），**236 条尚未核实**。
...
这个项目的文字量很大（1041 处地形 × 6 板块 + 691 座城市 × 7 段 + 295 条航线 × 2 套解说，
...
| 航线的**航班号 / 机型** | **低（默认不显示）** | 只有 59/295 条核实过；其余在界面、搜索、解说里一律不出现 |
...
lib/
  ...
  routes.ts / route-narration.ts — 航线（当前 295 条，国内+国际；航班号仅在核实过时对外显示）
                                    + 每条学习/旅游两套连贯解说
...
data/
  *.json                    — 早期地形数据（坐标现由 terrain-registry 覆盖）
  routes/                   — 航线定义（当前 295 条，国内+国际，含 source 核实留痕）
...
| 航班号 / 机型 | 写入时查得的示例；只有 `source` 留痕核实过的才对外显示（现 59/295），且只是核实当日的快照 |
