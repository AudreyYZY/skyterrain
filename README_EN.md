...
|---|---|---|
| **Study** | "What landform is that? How did it form?" | Terrain atlas — 246 landforms + authoritative 6-section lessons |
...
**Current scope:**
- Study — Asia (China 84 incl. Xinjiang + Japan 26), Oceania (Australia 22 + New Zealand 30), North America (USA 26 + Canada 25), Europe (UK 33) — 246 total, bilingual
...
- **246 landforms** registered in [`lib/terrain-registry.ts`](lib/terrain-registry.ts), 15 categories:
...
- Geometry self-check: `node --experimental-strip-types scripts/check-terrain-camera.ts` (246/246).
...
- **All 246 have bilingual lessons.** 61 of them (China 34 + Australia 22 + Xinjiang oasis settlements 5)
...
node --experimental-strip-types scripts/check-terrain-camera.ts   # camera geometry self-check (246/246)
...
lib/
  terrain-registry.ts       — [single source of truth] anchor/extent/axis/names/source for 246 terrains
