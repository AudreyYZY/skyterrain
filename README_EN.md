...
|---|---|---|
| **Study** | "What landform is that? How did it form?" | Terrain atlas — 274 landforms + authoritative 6-section lessons |
...
- **274 landforms** registered in [`lib/terrain-registry.ts`](lib/terrain-registry.ts), 15 categories:
...
- Geometry self-check: `node --experimental-strip-types scripts/check-terrain-camera.ts` (274/274).
...
node --experimental-strip-types scripts/check-terrain-camera.ts   # camera geometry self-check (274/274)
...
lib/
  terrain-registry.ts       — [single source of truth] anchor/extent/axis/names/source for 274 terrains
