...
export interface RouteSource {
   ...
   *
   * "flight-unknown" = 查过了，**机场对与承运人属实，但航班号/机型/班期拿不到可核实的来源**。
   * 这类路线 `flight` 字段可以整个缺省（不得填一个「看起来合理」的航班号顶上去），
   * 界面与搜索同样不显示航班信息，地理解说照常播。
   * 它与 "wrong" 的区别在于「记录本身有误」和「根本没拿到记录」是两件事，
   * 下一轮的入手点也不同：前者要找替代号，后者要找时刻表。
   */
  status?: "verified" | "wrong" | "flight-unknown";
